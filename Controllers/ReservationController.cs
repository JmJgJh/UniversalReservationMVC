...
namespace UniversalReservationMVC.Controllers
{
    public class ReservationController : Controller
    {
        ...
        public async Task<IActionResult> Create(int resourceId, int? seatId, int? eventId, string? returnUrl = null)
        {
            ...
            else if (!string.IsNullOrEmpty(resource.WorkingHours))
            {
                // Try to set times based on working hours
                try
                {
                    var workingHours = System.Text.Json.JsonSerializer.Deserialize<Dictionary<string, Dictionary<string, string>>>(resource.WorkingHours);
                    var dayName = now.DayOfWeek.ToString().ToLower();
                    
                    if (workingHours != null && workingHours.ContainsKey(dayName))
                    {
                        var hours = workingHours[dayName];
                        if (hours.ContainsKey("open") && hours.ContainsKey("close"))
                        {
                            var openTime = TimeSpan.Parse(hours["open"]);
                            var closeTime = TimeSpan.Parse(hours["close"]);
                            
                            var startDate = now.Date.Add(openTime);
                            if (startDate < now) startDate = now.AddHours(1); // If opening time passed, start in 1 hour
                            
                            vm.StartTime = startDate;
                            vm.EndTime = startDate.AddHours(1); // Default 1 hour duration
                        }
                    }
                }
                catch
